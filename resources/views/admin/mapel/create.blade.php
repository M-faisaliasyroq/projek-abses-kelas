...
@section('content')
<div class="mb-6">
    <h2 class="text-xl font-semibold text-gray-800 dark:text-slate-100">Tambah Mata Pelajaran</h2>
    <p class="text-sm text-gray-500 dark:text-slate-400">Tambahkan mata pelajaran baru</p>
</div>
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-200 dark:bg-slate-800 dark:border-slate-700 p-6 max-w-2xl">
    <form method="POST" action="{{ route('admin.mapel.store') }}">
        ...
        <x-input name="kode_mapel" label="Kode Mapel" required placeholder="Contoh: RPL-01"/>
        <x-input name="nama_mapel" label="Nama Mata Pelajaran" required placeholder="Contoh: Pemrograman Web"/>
        <x-textarea name="deskripsi" label="Deskripsi" rows="3" placeholder="Penjelasan singkat tentang mapel ini"/>
