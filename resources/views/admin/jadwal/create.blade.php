...
@section('content')
<div class="mb-6">
    <h2 class="text-xl font-semibold text-gray-800 dark:text-slate-100">Tambah Jadwal</h2>
    <p class="text-sm text-gray-500 dark:text-slate-400">Tentukan guru, mapel, kelas, dan jam mengajar</p>
</div>
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-200 dark:bg-slate-800 dark:border-slate-700 p-6 max-w-2xl">
    <form method="POST" action="{{ route('admin.jadwal.store') }}">
        ...
        <x-select name="guru_id" label="Guru" :options="$guru->mapWithKeys(fn($g) => [$g->id => $g->user->name])" placeholder="Pilih guru" required/>
        <x-select name="mata_pelajaran_id" label="Mata Pelajaran" :options="$mapel->mapWithKeys(fn($m) => [$m->id => $m->kode_mapel.' - '.$m->nama_mapel])" placeholder="Pilih mapel" required/>
        <x-select name="kelas" label="Kelas" :options="['X-RPL' => 'X-RPL', 'XI-RPL' => 'XI-RPL', 'XII-RPL' => 'XII-RPL']" placeholder="Pilih kelas" required/>

        <x-select name="hari" label="Hari" :options="['senin' => 'Senin', 'selasa' => 'Selasa', 'rabu' => 'Rabu', 'kamis' => 'Kamis', 'jumat' => 'Jumat']" placeholder="Pilih hari" required/>
